# Interstate-8.5 — Ticket Template

| | |
|---|---|
| **Document** | Ticket template and frontmatter specification |
| **Repository location** | `spec/tickets/TICKET-TEMPLATE.md` |
| **Applies to** | Every file in `spec/tickets/` (one ticket per file) |

This is the canonical format for tickets. Every ticket is a single Markdown file with the YAML
frontmatter and the body sections defined below. Tickets live in `spec/tickets/` beside the code, are
...
```yaml
---
id: INT8-000                # unique, zero-padded; see CONVENTIONS.md for the scheme
title: Short imperative summary
type: story                  # epic | story | task | spike
status: todo                 # todo | in-progress | blocked | in-review | done
milestone: 8                 # milestone number from spec/milestone-plan.md
...
depends_on: []               # ticket ids that must be done first, e.g. [INT8-003, INT8-004]
...
## References
- Specification sections (e.g. spec/requirements/requirements.md §4, spec/architecture/api-contract.md §2.x)
- Wireframes (spec/wireframes/…) and design system (spec/design/design-system.md)
```
...
- Placement/structure: spec/wireframes/NN-screen.md § (and state(s): empty/loading/error) — and the
  canonical hi-fi for the rendered composition
- Values (type, spacing, colour): spec/design/tokens.css via spec/design/design-system.md §2 — read
  `var(--…)`, never a hardcoded hex/px
...
(Most content-model, migration, services and tooling tickets are tasks.)

> **A task can still change what a person sees.** Much of this project's rendering work is `Task`-shaped —
> a Twig template, an SDC's markup, a block plugin, the ledger's grouping — and its appearance is still
> specified, so the `## Design authority` section below is **not** optional for it. "No UI" does not mean
> "no design".
...
fill in the `## QA steps` section. The chat response repeats this and additionally opens with the
ticket id, title, and its `## In plain English` line (see the definition of done in the root
`CLAUDE.md`).

```markdown
## Notes
- <DATE> — created
- <DATE> — in-review. Summary: <…>. Sanity test: `<command>`.
- <DATE> — done (reviewed).
```
