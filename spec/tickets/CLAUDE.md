...
- **`## Notes` quotes the red you saw — the test's name and its failure message, verbatim.**
  ...
  *"All tests passed first time"* does not discharge it: that is equally true of a test written from the
  spec beforehand and one written afterwards to fit working code. **This is the one definition-of-done
  item that cannot be audited later** — a gate re-runs and an amendment can be re-read, but *when* the
  failing test was written leaves no artefact unless the work commit captures one. Copy the message the
  moment you see red. If there is honestly no red, name the permitted substitute (root `CLAUDE.md` ›
  *Definition of done*) — **never manufacture a failure to satisfy the form**.
