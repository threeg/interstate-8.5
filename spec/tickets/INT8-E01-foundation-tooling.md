...
## Summary
Delivers the buildable environment: DDEV, Drupal 11, the essential contrib + admin theme, the owned
theme with Tailwind + tokens, the test/quality gate, and the v2 database mounted for import. This is
Milestone 8 (scaffolding).

## Scope
- **In scope:** repo + DDEV, Drupal install, essential contrib, Gin admin theme, starterkit theme +
...
## Success criteria
All children done; `ddev start` brings the site up; the empty `ddev test` gate passes; the dependency
rule is enforced.

## Children
- INT8-001 — Initialise repo + DDEV environment
