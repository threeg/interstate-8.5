# Interstate-8.5 — Ticket-System Conventions
...
> Replace `INT8` throughout with your project's short prefix (e.g. `ACME`, `WIDGET`). Replace the layer
...
1. **Implementation tickets** are `INT8-NNN`, zero-padded to three digits: `INT8-001`,
   `INT8-002`, … The number is allocated in **execution order** — a ticket with a lower number
...
2. **Epics** are `INT8-E0N`: `INT8-E01`, `INT8-E02`, … Epics are containers (brief capabilities)
...
3. The filename is the id plus a short slug: `INT8-007-core-constants.md`,
   `INT8-E02-pure-core.md`. One ticket per file.
...
| `layer` | The architecture layer (`content-model` \| `migration` \| `services` \| `theme` \| `config` \| `tooling` \| `docs` \| `repo`). The `depends_on` graph must respect the dependency rule for this layer (§4). The set is settled by the architecture (`spec/architecture/architecture.md` §2.1); keep the two identical. |
