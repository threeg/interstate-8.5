...
3. **An answer is a specification change, not a cleanup ticket.** When one lands, amend the owning `spec/`
   document **first** (and the `decisions.md` beside it), then change code to match. Never patch the code
   and leave the spec stale.
...
If an answer **changes** a value already built on, say so explicitly in the `decisions.md` beside the
owning document — a silently-corrected value is indistinguishable from a bug.
