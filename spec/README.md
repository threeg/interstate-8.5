...
2. **Install the workflow skills** in `.claude/skills/` (`sfk-init`, `sfk-version`,
   `sfk-next-milestone`, `sfk-signoff`, `sfk-next-ticket`, `sfk-verify`, `sfk-update-kit`,
   `sfk-feedback`):
...
├── .claude/
│   └── skills/                   ← the eight workflow skills (auto-discovered by Claude Code)
...
| 8 | **Scaffolding** | repo skeletons that compile and test, dependency-rule check wired in, **`sfk-verify` filled in for the stack** | Building (Code) | `sfk-next-milestone` → `sfk-signoff` |
...
You answer questions and sign off; the agent does the rest. Eight skills, mapped to the lifecycle:
...
| `sfk-next-ticket` | repeatedly, in step 8 | Finalizes the previous ticket left `in-review` (→ `done`), then implements the next ready `todo` ticket and leaves it `in-review` for you to review. |
