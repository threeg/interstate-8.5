# <PROJECT> — the architecture and the interface contract decisions
...
| **Status** | **Archive — not binding.** The binding document is `architecture.md` and `api-contract.md` |
...
- **Decisions** — architecture and contract decisions — the layering chosen and the alternatives rejected, why this stack suits these requirements, why a boundary sits where it does, and why a contract shape was fixed. Record the options considered and the reason, not just the outcome.
...
- **<DATE>** — `<component / endpoint / §n>` — <the decision, the options considered, and why this one>.
- **<DATE>** — `<component / endpoint / §n>` — amended in vX.Y.Z. Previously read: "<the exact superseded wording>".
  <Why it changed.>
